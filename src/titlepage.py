...
TEXT = (
    "### Written by Twelve Winged Dark Seraphim\n ### Complied and Edited by Max Ludden"
)
...
def generate_titlepage_filename(book: int, save: bool = True) -> str:
    ...
    if save:
        ...
        doc.save()
        log.debug(
            f"Saved Book {book}'s titlepage to MongoDB. Filename:<code>\n{filename}</code>"
        )

    console.print(titlepage_panel(book, "Filename", filename, 96), highlight=True)
    return filename
...
def generate_titlepage_md_path(book: int, save: bool = True) -> Path:
    ...
    if save:
        ...
        doc.save()
        log.debug(
            f"Saved Book {book}'s titlepage to MongoDB. Markdown path:<code>\n{md_path}</code>"
        )

    console.print(titlepage_panel(book, "Markdown Path", md_path, 140), highlight=True)
    return Path(md_path)
...
def get_titlepage_md_path(book: int) -> Path:
    ...
    if doc.md_path:
        ...
        console.print(
            titlepage_panel(book, "Markdown Path", doc.md_path, 151, get=True),
            highlight=True,
        )
...
def generate_titlepage_html_path(book: int, save: bool = True):
    ...
    if save:
        ...
        if doc:
            ...
            doc.save()
            log.debug(
                f"Saved Book {book}'s titlepage to MongoDB. HTML path:<code>\n{html_path}</code>"
            )
        else:
            raise TitlepageNotFound(f"Titlepage for Book {book} not found.")

    console.print(titlepage_panel(book, "HTML Path", html_path, 187))
...
def get_titlepage_html_path(book: int) -> Path:
    ...
    if doc.html_path:
        ...
        console.print(titlepage_panel(book, "HTML Path", doc.html_path, 209, get=True))
...
def generate_titlepage_md(book: int, save: bool = True, write=True) -> str | None:
    ...
    if doc:
        ...
        console.print(titlepage_panel(book, "Markdown", md, 268), highlight=True)
        return md
...
def get_titlepage_md(book: int) -> str:
    ...
    if doc.md:
        console.print(
            titlepage_panel(book, "Markdown", Markdown(doc.md), 278, get=True)
        )
        return doc.md
...
def generate_titlepage_html(book: int, save: bool = True) -> str | None:
    ...
    doc.md_path = str(generate_titlepage_md_path(book))
    doc.html_path = str(generate_titlepage_html_path(book))
    doc.save()
    html = save_titlepage_html(mmd(doc.html_path, doc.md_path), book)  # type: ignore
    footer = """ 	<div class="footer">
		<h3>Written by Twelve Winged Dark Seraphim</h3>
    ...
    console.print(titlepage_panel(book, "HTML", f"\n\n{html}", 334))
    return html


def generate_titlepage_book_word(book: int, save: bool = False) -> str:
    """
    Generate the written version of the given book's number.

    Args:
        `book` (int): The given book.

    Returns:
        `book_word` (str): The written version of the given book's number.
    """
    ...
    if save:
        ...
        doc.save()
        log.debug(f"Saved Book {book}'s titlepage's book word to MongoDB.")
