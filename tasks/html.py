...
def generate_html(chapter: int, save: bool = True):
    ...
    if doc:
        ...
        console.log(
            Panel(
                Text(f'Markdown Path: {md_path}\nHTML Path: {html_path}', justify='left', style='white'),
                title=Text(f'Paths', style='bold green'),
                title_align='left',
                expand=False,
                border_style='#00ff00',
            )
        ...
        if md_path.exists():
            ...
            if result.exit_code == 0: # type: ignore
                console.log(
                    Panel(
                        Text(f"Successfully generated HTML for doc {doc.chapter}.",
                        justify="left",
                        style="white",
                        ),
                        title=Text(
                ...
                if save:
                    ...
                    with open(html_path, 'r') as infile:
                        ...
                        console.log(
                            Panel(
                                Text(
                                    f"Saved Chapter {doc.chapter}'s HTML to MongoDB.",
                                    justify="left",
                                    style="white"
                                ),
                                title=Text(
    ...
    log.debug(f"Generated HTML for Chapter {doc.chapter} in {end - start:0.4f} seconds.")

if __name__ == '__main__':
    with HtmlProgress() as progress:
