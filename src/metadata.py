...
def get_meta_filename(book: int) -> str:
    ...
    if doc.filename:
        logpanel(
            f"Retrieved Book {book}'s Metadata's filename from MongoDB.", level="d"
        )
        return doc.filename
    else:
...
def get_meta_filepath(book: int) -> Path:
    ...
    if doc:
        logpanel(
            f"Retrieved Book {book}'s Metadata's filepath from MongoDB.", level="d"
        )
        return doc.filepath
    else:
...
def generate_metadata(book: int, save: bool = False, write: bool = False):
    ...
    doc = Metadata.objects(book=book).first()  # type: ignore
    # > Generate Text
    metadata_dict = {"title": doc.title, "author": author}
